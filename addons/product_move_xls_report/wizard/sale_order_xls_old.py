...
class SaleOrderReport(models.TransientModel):
    ...
    def action_sale_report(self):
        ...
        if len(self._context.get('active_ids', [])) > 0:
            ...
            inventory_adjacetment = self.env['stock.quant'].search(
        ...
        if product_move:
            ...
            sheets.write(row, 6, 'Balance', format2)
            sheets.write(row, 7, 'Partner', format2)
            sheets.write(row, 8, 'Quantity Done', format2)
            sheets.write(row, 9, 'Status', format2)
            sheets.write(row, 10, 'Cost', format2)
            sheets.write(row, 11, 'Out', format2)
            sheets.write(row, 12, 'In', format2)
            sheets.write(row, 13, 'Source Document', format2)
            ...
            for counter, rec in enumerate(product_move):
                ...
                # if rec.location_id.complete_name.split()[-1] == 'adjustment':
                #    sheets.write(row + 1, count + 6, inventory_adjacetment.quantity, format2)
                # else:
                sheets.write(row + 1, count + 6, rec.done_qty_2 + done_quan, format2)

                sheets.write(row + 1, count + 7, rec.my_partner.name, format2)
                ...
                if wizard.stock_location_id.id == rec.location_id.id:
                    sheets.write(row + 1, count + 11, round(cost, 2), format2)
                else:
                    sheets.write(row + 1, count + 11, '', format2)

                if wizard.stock_location_id.id == rec.location_dest_id.id:
                    sheets.write(row + 1, count + 12, round(cost, 2), format2)
                else:
                    sheets.write(row + 1, count + 12, '', format2)

                # sheets.write(row + 1, count + 13, rec.picking_id.origin1.name if rec.picking_id.origin1.name else '',
                #              format2)
                row += 1
        else:
