...
def report_header(workbook, sheet):
    ...
    sheet.write_merge(20, 21, 1, 1, 'الفترة', style=style_a2a2a2)
    sheet.write_merge(20, 21, 2, 2, 'السنة', style=style_a2a2a2)
    sheet.write_merge(20, 21, 3, 3, 'رقم التسجيل الضريبى', style=style_a2a2a2)
    sheet.write_merge(20, 21, 4, 4, 'رقم الملف', style=style_a2a2a2)
    ...
    sheet.write_merge(20, 21, 16, 16, 'اسم الممول', style=style_a2a2a2)
    sheet.col(4).width = 6000
    sheet.write_merge(20, 21, 17, 17, 'العنوان', style=style_a2a2a2)
    sheet.col(14).width = 1500
    sheet.write_merge(20, 21, 18, 18, 'الرقم القومي', style=style_a2a2a2)
    sheet.write_merge(20, 21, 19, 19, 'العملة', style=style_a2a2a2)
...
def write_dictionary_in_sheet(margin, sheet, dict_data):
    ...
    for value in dict_data.values():
        ...
        sheet.write(margin + count + 1, 1, "الفترة الأولى" if 1 <= datetime.datetime.strptime(value['date_invoice'],
                                                                                               '%d-%m-%Y').month <= 3 else "الفترة الثانية" if 4 <= datetime.datetime.strptime(
            value['date_invoice'], '%d-%m-%Y').month <= 6 else "الفترة الثالثة" if 7 <= datetime.datetime.strptime(
            value['date_invoice'], '%d-%m-%Y').month <= 9 else "الفترة الرابعة")
        sheet.write(margin + count + 1, 2, datetime.datetime.now().year)

        sheet.write(margin + count + 1, 3, value['partner_vat'] if value['partner_vat'] else '')
        sheet.write(margin + count + 1, 4, value['partner_tax_file'] if value['partner_tax_file'] else '')
        ...
        sheet.write(margin + count + 1, 16, value['partner_name'] if value['partner_name'] else '')
        sheet.write(margin + count + 1, 17, value['partner_address'] if value['partner_address'] else '')
        sheet.write(margin + count + 1, 18, value['national_id'] if value['national_id'] else '')
            ...
            for invoice_line in progress_invoice.invoice_line_ids:
                for tax_line in invoice_line.invoice_line_tax_ids:
                    ...
                    if tax_type == 'أ.ت.ص' and invoice_line.price_subtotal != 0:
                        ...
                        partner_tax_type_key = tax_line.name + '-' + str(
                            progress_invoice.partner_id.id) + '-' + interval_date
                        ...
                        if partner_tax_type_key in report_dict.keys():
                            ...
                            tmp_values['national_id'] = progress_invoice.partner_id.national_id
