...
class journal_entry_inheriy(models.Model):
    ...
    journal_parent = fields.Many2one('account.batch.payment', string="Journal parent")
...
class cheque(models.Model):
        ...
        if 'state' in vals:
            if vals['state']=='matched':
                if self.batch_payment_id:
                    if self.batch_payment_id.cheque_type == 'out':
                        self.batch_payment_id.state_out = 'matched'

                    elif self.batch_payment_id.cheque_type == 'in':
                        self.batch_payment_id.state_in = 'matched'
        # print(self, vals, result)
        # self.hide_payment_method = False

        return result
    # def post(self):
    #     res = super(cheque, self).post()
    #     if(self.journal_id.name == 'Cheque'):
    #         pays = [[4, self.id]]
    #         if self.communication.startswith('BILL'):
    #             obj = self.env['account.batch.deposit'].create({
    #                 'cheque_type': 'out',
    #                 'name': self.communication,
    #                 'date': fields.Date.today(),
    #                 'journal_id': self.journal_id.id,
    #                 'account_1': self.destination_account_id.id,
    #                 'account_2': self.account.id,
    #                 'state_of_cheque2': 'to_client',
    #                 'cheque_number': self.cheque_hash,
    #                 'maturity_date': self.cheque_date,
    #                 'payment_ids': pays,
    #                 'status_date': fields.Datetime.now()
    #             })
    #             obj.move_name = self.move_name
    #         else:
    #             obj = self.env['account.batch.deposit'].create({
    #                 'cheque_type': 'in',
    #                 'journal_id': self.journal_id.id,
    #                 'account_1': self.destination_account_id.id,
    #                 'account_2': self.account.id,
    #                 'state_of_cheque': 'cheque_under_collection',
    #                 'cheque_number': self.cheque_hash,
    #                 'maturity_date': self.cheque_date,
    #                 'payment_ids': pays,
    #                 'status_date': fields.Datetime.now()
...
class AddAccounts(models.Model):
    ...
    # @api.one
    # @api.constrains('journal_id', 'payment_ids','cheque_type')
    ...
    #     if any(payment.partner_type == 'customer' and self.cheque_type =='out' for payment in self.payment_ids):
    #
    ...
    # @api.model
    # def create(self, vals):
    #     result = super(AddAccounts, self).create(vals)
    #     print("vals['cheque_type']",vals['cheque_type'])
    #     print("vals['payment_ids'][0]",vals['payment_ids'] )
    #     if vals['cheque_type'] == 'out' and vals['payment_ids'][0][2].partner_type=='customer' :
    #         raise UserError(_('You must choose partner type vendor if cheque type is in '))
    #
    #
    #
    #     return result
    # my_company_id = fields.Many2one('res.company', string='Company', default=lambda self: self.env.user.company_id.id)


    # journal_id = fields.Many2one('account.journal', string='Bank', domain=[('company_id','=',1)], required=True,readonly=True,oldname='journal_id', states={'draft': [('readonly', False)]})
    ...
    def _get_default_journal(self):
        ...
        cheque_draft_journal_id = IrDefault.get(
            'account.batch.payment', 'cheque_draft_journal_id', company_id=self.env.user.company_id.id)
    ...
    def create_journal_entry(self):
        ...
        if self.state_in == 'cheques_in_safe':
            ...
            intermediate_account = IrDefault.get(
                'account.batch.payment','account_id_receipt_intermediate', company_id=self.env.user.company_id.id)
            transfer_account = IrDefault.get(
                'account.batch.payment','account_id_transfer_intermediate', company_id=self.env.user.company_id.id)
    ...
    def pay_close(self):
        ...
        IrDefault = self.env['ir.default'].sudo()
        intermediate_account = IrDefault.get(
            'account.batch.payment', 'account_id_receipt_intermediate_out', company_id=self.env.user.company_id.id)
        aml_obj = self.env['account.move.line'].with_context(
            check_move_validity=False)
        values = self.get_move_vals()
        values.update({'ref': self.name,
    ...
    def make_close(self):
        ...
        pay_debit_setting = IrDefault.get(
            'account.batch.payment', 'pay_debit_setting', company_id=self.env.user.company_id.id)
        pay_credit_setting = IrDefault.get(
            'account.batch.payment', 'pay_credit_setting', company_id=self.env.user.company_id.id)
...
class config_cheque(models.TransientModel):
    ...
    def get_values(self):
        ...
        #ICPSudo = self.env['ir.config_parameter'].sudo()
        cheque_draft_journal_id= IrDefault.get(
            'account.batch.payment', 'cheque_draft_journal_id', company_id = self.company_id.id or self.env.user.company_id.id)
        intermediate_account = IrDefault.get(
            'account.batch.payment','account_id_receipt_intermediate', company_id=self.company_id.id or self.env.user.company_id.id)

        transfer_account = IrDefault.get(
            'account.batch.payment','account_id_transfer_intermediate', company_id=self.company_id.id or self.env.user.company_id.id)
        intermediate_account_out = IrDefault.get(
            'account.batch.payment', 'account_id_receipt_intermediate_out',
            company_id=self.company_id.id or self.env.user.company_id.id)

        transfer_account_out = IrDefault.get(
            'account.batch.payment', 'account_id_transfer_intermediate_out',
            company_id=self.company_id.id or self.env.user.company_id.id)

        rec_account_1 = IrDefault.get(
            'account.batch.payment', 'rec_account_1',
            company_id=self.company_id.id or self.env.user.company_id.id)

        rec_account_2 = IrDefault.get(
            'account.batch.payment', 'rec_account_2',
            company_id=self.company_id.id or self.env.user.company_id.id)
        pay_debit_setting = IrDefault.get(
            'account.batch.payment', 'pay_debit_setting',
            company_id=self.company_id.id or self.env.user.company_id.id)

        pay_credit_setting = IrDefault.get(
            'account.batch.payment', 'pay_credit_setting',
            company_id=self.company_id.id or self.env.user.company_id.id)
    ...
    def set_values(self):
        ...
        IrDefault = self.env['ir.default'].sudo()
        IrDefault.set(
            'account.batch.payment', 'cheque_draft_journal_id', self.cheque_draft_journal_id.id, company_id=self.company_id.id)
        IrDefault.set(
            'account.batch.payment','account_id_receipt_intermediate', self.intermediate_account.id,company_id=self.company_id.id)
        IrDefault.set(
            'account.batch.payment','account_id_transfer_intermediate', self.transfer_account.id,company_id=self.company_id.id)
        IrDefault.set(
            'account.batch.payment', 'account_id_receipt_intermediate_out', self.intermediate_account_out.id,
            company_id=self.company_id.id)
        IrDefault.set(
            'account.batch.payment', 'account_id_transfer_intermediate_out', self.transfer_account_out.id,
            company_id=self.company_id.id)

        IrDefault.set(
            'account.batch.payment', 'rec_account_1', self.rec_account_1.id,
            company_id=self.company_id.id)
        IrDefault.set(
            'account.batch.payment', 'rec_account_2', self.rec_account_2.id,
            company_id=self.company_id.id)
        IrDefault.set(
            'account.batch.payment', 'pay_debit_setting', self.pay_debit_setting.id,
            company_id=self.company_id.id)
        IrDefault.set(
            'account.batch.payment', 'pay_credit_setting', self.pay_credit_setting.id,
            company_id=self.company_id.id)
