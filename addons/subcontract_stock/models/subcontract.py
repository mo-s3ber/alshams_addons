...
from odoo.addons.subcontract.models.subcontract import SubcontractOrder as Subcontract
...
class SubcontractOrder(models.Model):
    ...
    picking_type_id = fields.Many2one('stock.picking.type', 'Deliver To', states=Subcontract.READONLY_STATES, required=True, default=_default_picking_type,
    ...
    def button_approve(self, force=False):
        result = super(SubcontractOrder, self).button_approve(force=force)
    ...
    def button_cancel(self):
        for order in self:
            for pick in order.picking_ids:
                if pick.state == 'done':
                    raise UserError(_('Unable to cancel subcontract order %s as some receptions have already been done.') % (order.name))
            # If the product is MTO, change the procure_method of the the closest move to subcontract to MTS.
        ...
        return super(SubcontractOrder, self).button_cancel()

    @api.multi
    def action_view_picking(self):
        """ This function returns an action that display existing picking orders of given subcontract order ids. When only one found, show the picking immediately.
        """
    ...
    def _log_decrease_ordered_quantity(self, subcontract_order_lines_quantities):
        ...
        def _render_note_exception_quantity_po(order_exceptions):
            order_line_ids = self.env['subcontract.order.line'].browse([order_line.id for order in order_exceptions.values() for order_line in order[0]])
            subcontract_order_ids = order_line_ids.mapped('order_id')
            ...
            values = {
                'subcontract_order_ids': subcontract_order_ids,
                'order_exceptions': order_exceptions.values(),
                'impacted_pickings': impacted_pickings,
            }
            return self.env.ref('subcontract_stock.exception_on_po').render(values=values)

        documents = self.env['stock.picking']._log_activity_get_documents(subcontract_order_lines_quantities, 'move_ids', 'DOWN', _keys_in_sorted, _keys_in_groupby)
        filtered_documents = {}
...
class SubcontractOrderLine(models.Model):
    ...
    move_ids = fields.One2many('stock.move', 'subcontract_line_id', string='Reservation', readonly=True, ondelete='set null', copy=False)
    orderpoint_id = fields.Many2one('stock.warehouse.orderpoint', 'Orderpoint')
    move_dest_ids = fields.One2many('stock.move', 'created_subcontract_line_id', 'Downstream Moves')
    ...
    def write(self, values):
        result = super(SubcontractOrderLine, self).write(values)
        # Update expected date of corresponding moves
        if 'date_planned' in values:
            self.env['stock.move'].search([
                ('subcontract_line_id', 'in', self.ids), ('state', '!=', 'done')
            ]).write({'date_expected': values['date_planned']})
        if 'product_qty' in values:
            self.filtered(lambda l: l.order_id.state == 'subcontract')._create_or_update_picking()
        return result
            ...
            if line.product_id.type in ('product', 'consu'):
                ...
                if float_compare(line.product_qty, line.qty_invoiced, line.product_uom.rounding) == -1:
                    ...
                    activity = self.env['mail.activity'].sudo().create({
                        ...
                        'note': _('The quantities on your subcontract order indicate less than billed. You should ask for a refund. '),
