...
class AccountInvoice(models.Model):
    ...
    @api.model
    def _anglo_saxon_subcontract_move_lines(self, i_line, res):
        """Return the additional move lines for subcontract invoices and refunds.
        ...
        if i_line.product_id and i_line.product_id.valuation == 'real_time' and i_line.product_id.type == 'product':
            ...
            for line in res:
                if line.get('invl_id', 0) == i_line.id and reference_account_id == line['account_id']:
                    ...
                    if i_line.product_id.cost_method != 'standard' and i_line.subcontract_line_id:
                        co_currency = i_line.subcontract_id.currency_id
                        co_company = i_line.subcontract_id.company_id
                        #for average/fifo/lifo costing method, fetch real cost price from incomming moves
                        valuation_price_unit = co_currency._convert(
                            i_line.subcontract_line_id.product_uom._compute_price(i_line.subcontract_line_id.price_unit, i_line.uom_id),
                            inv.currency_id,
                            company=co_company, date=inv.date or inv.date_invoice, round=False,
                        )
                        stock_move_obj = self.env['stock.move']
                        valuation_stock_move = stock_move_obj.search([
                            ('subcontract_line_id', '=', i_line.subcontract_line_id.id),
                        ...
                        if valuation_stock_move:
                            ...
                            valuation_price_unit = co_currency._convert(
                                i_line.subcontract_line_id.price_unit, inv.currency_id,
    ...
    def _get_last_step_stock_moves(self):
        ...
        for invoice in self.filtered(lambda x: x.type == 'in_invoice'):
            rslt += invoice.mapped('invoice_line_ids.subcontract_line_id.move_ids').filtered(lambda x: x.state == 'done' and x.location_id.usage == 'supplier')
        for invoice in self.filtered(lambda x: x.type == 'in_refund'):
            rslt += invoice.mapped('invoice_line_ids.subcontract_line_id.move_ids').filtered(lambda x: x.state == 'done' and x.location_dest_id.usage == 'supplier')
        return rslt
