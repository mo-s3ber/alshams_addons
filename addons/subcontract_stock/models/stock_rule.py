...
class StockRule(models.Model):
    ...
    def _run_buy(self, product_id, product_qty, product_uom, location_id, name, origin, values):
        ...
        if not suppliers:
            msg = _('There is no subcontractor associated to the product %s. Please define a subcontractor for this product.') % (product_id.display_name,)
            raise UserError(msg)   
        supplier = self._make_co_select_supplier(values, suppliers)
        ...
        if domain in cache:
            ...
            co = self.env['subcontract.order'].sudo().search([dom for dom in domain])
            co = co[0] if co else False
            cache[domain] = co
        if not co:
            vals = self._prepare_subcontract_order(product_id, product_qty, product_uom, origin, values, partner)
            company_id = values.get('company_id') and values['company_id'].id or self.env.user.company_id.id
            co = self.env['subcontract.order'].with_context(force_company=company_id).sudo().create(vals)
            cache[domain] = co
        elif not co.origin or origin not in co.origin.split(', '):
            if co.origin:
                if origin:
                    co.write({'origin': co.origin + ', ' + origin})
                else:
                    co.write({'origin': co.origin})
            else:
                co.write({'origin': origin})
        ...
        for line in co.order_line:
            if line.product_id == product_id and line.product_uom == product_id.uom_po_id:
                if line._merge_in_existing_line(product_id, product_qty, product_uom, location_id, name, origin, values):
                    vals = self._update_subcontract_order_line(product_id, product_qty, product_uom, values, line, partner)
                    co_line = line.write(vals)
                    break
        if not co_line:
            vals = self._prepare_subcontract_order_line(product_id, product_qty, product_uom, values, co, partner)
            self.env['subcontract.order.line'].sudo().create(vals)

    def _get_subcontract_schedule_date(self, values):
        """Return the datetime value to use as Schedule Date (``date_planned``) for the
           Subcontract Order Lines created to satisfy the given procurement. """
        procurement_date_planned = fields.Datetime.from_string(values['date_planned'])
        schedule_date = (procurement_date_planned - relativedelta(days=values['company_id'].co_lead))
        return schedule_date

    def _get_subcontract_order_date(self, product_id, product_qty, product_uom, values, partner, schedule_date):
        """Return the datetime value to use as Order Date (``date_order``) for the
           Subcontract Order created to satisfy the given procurement. """
    ...
    def _update_subcontract_order_line(self, product_id, product_qty, product_uom, values, line, partner):
    ...
    @api.multi
    def _prepare_subcontract_order_line(self, product_id, product_qty, product_uom, values, co, partner):
        procurement_uom_po_qty = product_uom._compute_quantity(product_qty, product_id.uom_po_id)
        seller = product_id._select_seller(
            partner_id=partner,
            quantity=procurement_uom_po_qty,
            date=co.date_order and co.date_order.date(),
            uom_id=product_id.uom_po_id)

        taxes = product_id.supplier_taxes_id
        fpos = co.fiscal_position_id
        ...
        if price_unit and seller and co.currency_id and seller.currency_id != co.currency_id:
            price_unit = seller.currency_id._convert(
                price_unit, co.currency_id, co.company_id, co.date_order or fields.Date.today())
        ...
        date_planned = self.env['subcontract.order.line']._get_date_planned(seller, po=po).strftime(DEFAULT_SERVER_DATETIME_FORMAT)
    ...
    def _prepare_subcontract_order(self, product_id, product_qty, product_uom, origin, values, partner):
        schedule_date = self._get_subcontract_schedule_date(values)
        subcontract_date = self._get_subcontract_order_date(product_id, product_qty, product_uom, values, partner, schedule_date)
        ...
        return {
            ...
            'currency_id': partner.with_context(force_company=values['company_id'].id).property_subcontract_currency_id.id or self.env.user.company_id.currency_id.id,
            ...
            'date_order': subcontract_date.strftime(DEFAULT_SERVER_DATETIME_FORMAT),
            'fiscal_position_id': fpos,
            'group_id': group
        }

    def _make_co_select_supplier(self, values, suppliers):
